...
--sdl.setHint('SDL_HINT_IDLE_TIMER_DISABLED','1')
--sdl.setHint('SDL_RENDERER_PRESENTVSYNC','1')
--sdl.setHint('SDL_HINT_RENDER_DRIVER','software') --direct3d opengl opengles2 opengles software 
--sdl.setHintWithPriority('SDL_HINT_RENDER_DRIVER','software','SDL_HINT_OVERRIDE')
--sdl.setHintWithPriority('SDL_HINT_VIDEO_HIGHDPI_DISABLED','1','SDL_HINT_OVERRIDE')
...
-----------------------------------------------------------------------
 ...
 -- local sdlWindow--sdl.gL_GetCurrentWindow() --тебя нужно вызыввать лишь один раз при запуске... так что оставлю вызов здеся.
  local wndsurf = 0
  local drawbuf=  0--sdl.createRGBSurface(0,width,heigh,32,0,0,0,0) -я передумал использовать блиттинг и буду рисовать сразу на повехрности окна
  ...
  local function init (w,h)
   -- width,heigh=wd,hg -- вообще-то, sdlWindow уже содержит актуальные w,h  
	...
	drawbuf=  sdl.createRGBSurface(0,w,h,32,0,0,0,0)
	rect=sdl.Rect(0,0,w,h)
    buf[0]=ffi.cast("uint32_t *",drawbuf.pixels)
    bufrgba[0]=ffi.cast('rgba_pixel *',drawbuf.pixels)
	for i=1,heigh-1 do
		buf[i]=buf[0]+width*i
